...
#define CONSTANT_STRING_NUM 121
#define CONSTANT_CHARACTER "字符"
#define CONSTANT_CHARACTER_NUM 122
#define NOTE "注释"
#define NOTE_NUM 140
...
void preProcess(char *str,int line){
    ...
    if(s_include != NULL){
        ...
        addNewNode(s_include+i,HEADER,130,-1,line);
    }
    else{   //判断宏定义
        ...
        addNewNode(s_define+i,MACRO,131,-1,line);
    }
...
void lexical_analysis(){
    ...
    FILE *input_file;
    printf("Please input the C program:");
    scanf("%s",filename);
        ...
        //以"/"开头
            ...
            if(character == '='){
             ...
             //处理"/**/"形注释
                ...
                addNewNode("NULL",NOTE,NOTE_NUM,-1,line);
            }
            //处理"//"型注释
            else if(character == '/'){
                while(character != '\n'){
...
void printResult(Node *node){
    ...
    cout<<setw(30)<<"内容"<<setw(10)<<"描述"<<"\t"<<"种别码"<<"\t"<<"内码值"<<"\t"<<"行号"<<endl;
    while(node != NULL){
